...
try:
    ...
    gemini_api_key = os.environ.get("GOOGLE_API_KEY", "AIzaSyD5odSXjvSnSUkG6scYidq6_mapUN7FSCE") # Prioritize env var
    if not gemini_api_key:
        raise ValueError("GOOGLE_API_KEY not found in environment or hardcoded.")
    genai.configure(api_key=gemini_api_key)
    # Initialize the Gemini model
    gemini_model = genai.GenerativeModel('gemini-2.0-flash-lite-001') # Consider making model configurable
...
generate_reply_tool = send_reply
send_reply_tool = send_reply
search_emails_tool = search_emails
generate_reply_with_gemini = generate_reply_with_gemini
